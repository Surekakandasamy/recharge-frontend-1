...
export default function Admin() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        const response = await fetch('https://recharge-backend-2daw.onrender.com/api/users');
    ...
    const loadTransactions = async () => {
      try {
        const response = await fetch('https://recharge-backend-2daw.onrender.com/api/transactions');
        const data = await response.json();
  ...
  const handleAddPlan = async (e) => {
    ...
    try {
      const response = await fetch('https://recharge-backend-2daw.onrender.com/api/plans', {
        method: 'POST',
  ...
  const handleUpdatePlan = async (e) => {
    ...
    try {
      const response = await fetch(`https://recharge-backend-2daw.onrender.com/api/plans/${editingPlan._id || editingPlan.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeletePlan = async (planId) => {
    if (confirm('Are you sure you want to delete this plan?')) {
      try {
        const response = await fetch(`https://recharge-backend-2daw.onrender.com/api/plans/${planId}`, {
          method: 'DELETE'
        });
